...
import { AlertCircle, Bot, CheckCircle2, ChevronDown, Copy, FileText, GitBranch, Globe2, Menu, MessageSquarePlus, PanelRight, Pencil, Plus, RefreshCw, Search, Send, ShieldCheck, Sparkles, Square, Trash2, X } from 'lucide-react'
...
export default function App() {
  ...
  async function confirmDialog(value: string) {
    ...
    if (dialog.kind === 'rename') {
      ...
      await saveConversation({ id: dialog.id, title: value.slice(0, 120), model: conversation.model, messages: conversation.messages })
      await refreshHistory()
      setDialog(null)
      return
    }
  ...
  return <div className="app-shell">
    ...
    <aside className={`context-panel ${contextOpen ? 'is-open' : ''}`} aria-label="Conversation context"><div className="context-heading"><div><strong>Context</strong><span>Conversation resources</span></div><button className="icon-button" onClick={() => setContextOpen(false)} aria-label="Close context panel"><X size={19}/></button></div><div className="context-card"><span className="context-card-icon"><ShieldCheck size={19}/></span><div><strong>Private processing</strong><p>This conversation is configured for the local Ollama runtime.</p></div></div><div className="context-card"><span className="context-card-icon"><GitBranch size={19}/></span><div><strong>Lineage</strong><p>{currentSummary?.parentConversationId ? `Branched from conversation ${currentSummary.parentConversationId.slice(0, 8)} at message ${Number(currentSummary.parentMessageIndex) + 1}.` : 'This is a root conversation.'}</p></div></div><div className="context-card"><span className="context-card-icon"><FileText size={19}/></span><div><strong>Workspace</strong><p>No Workspace attached yet.</p></div></div></aside>
